...
public static class AdresseHelper
{
    ...
    {
        var callback = Addressables.LoadAssetAsync<T>(adress);
        callback.Completed += (_) => OnLoadedAssetAsyncIndexed(adress, _, callbackAction, index);
    }
    
    static void OnLoadedAssetAsyncIndexed<T>(string key, AsyncOperationHandle<T> handle, Action<T,int> callbackAction, int index)
    {
        if (handle.Status == AsyncOperationStatus.Succeeded)
        {
