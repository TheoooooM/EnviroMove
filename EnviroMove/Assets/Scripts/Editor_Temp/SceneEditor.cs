...
public class SceneEditor
{
    ...
    private List<int> twoByOnePrefabIndex = new() { 24, 26, 27, 28, 43, 46, 47, 82, 84, 85, 86 };
    private List<int> threeByOnePrefabIndex = new() { 25, 81 };
    ...
    private void StoreBlockData(Vector3 position)
    {
        ...
        {
            if (blockGrid[(int)position.x + 2, (int)position.y, (int)position.z] == 0)
            {
                blockGrid[(int)position.x + 2, (int)position.y, (int)position.z] = 186;
                if (blockHorizontalRotationGrid[(int)position.x + 2, (int)position.y, (int)position.z] ==
                    Enums.Side.none)
                    blockHorizontalRotationGrid[(int)position.x + 2, (int)position.y, (int)position.z] =
                        Enums.Side.forward;
                if (blockVerticalRotationGrid[(int)position.x + 2, (int)position.y, (int)position.z] ==
                    Enums.Side.none)
                    blockVerticalRotationGrid[(int)position.x + 2, (int)position.y, (int)position.z] =
                        Enums.Side.forward;
            }
        }
    ...
    private void DeletePanel(GameObject newGo)
    {
        var posOfnewGo = newGo.transform.position;
        var rotationGo = (int)newGo.transform.rotation.eulerAngles.y;
        var posToInstantiateNewBorder = new Vector3();
        var posToInstantiateNewBorder2 = new Vector3();
        var newBorder = new GameObject();
        var newBorder2 = new GameObject();
        switch (rotationGo)
        {
            case 0:
                posToInstantiateNewBorder = new Vector3(posOfnewGo.x, posOfnewGo.y, posOfnewGo.z + 1);
                blockGrid[(int)posOfnewGo.x, 1, (int)posOfnewGo.z + 1] = (int)Enums.blockType.InsideTop1 + (int)(posToInstantiateNewBorder.x % (tileSize.x + tailleBridge));
                newBorder = Object.Instantiate(prefabs[blockGrid[(int)posOfnewGo.x, 1, (int)posOfnewGo.z + 1]],
                    new Vector3(posToInstantiateNewBorder.x - .5f,.5f, posToInstantiateNewBorder.z - .5f), Quaternion.identity);
                newBorder.transform.parent = parent.transform;
                newBorder.transform.Rotate(-90,-90,0);
                newBorder.name = "Block (" + (int)posToInstantiateNewBorder.x + ", " + 1 + ", " +
                                 (int)posToInstantiateNewBorder.z + ")";
                
                posToInstantiateNewBorder2 = new Vector3(posOfnewGo.x, posOfnewGo.y, posOfnewGo.z + 4);
                blockGrid[(int)posOfnewGo.x, 1, (int)posOfnewGo.z + 4] = (int)Enums.blockType.InsideBottom1 + (int)(posToInstantiateNewBorder.x % (tileSize.x + tailleBridge));
                newBorder2 = Object.Instantiate(prefabs[blockGrid[(int)posOfnewGo.x, 1, (int)posOfnewGo.z + 4]],
                    new Vector3(posToInstantiateNewBorder2.x + .5f,.5f, posToInstantiateNewBorder2.z + .5f), Quaternion.identity);
                newBorder2.transform.parent = parent.transform;
                newBorder2.transform.Rotate(-90,0,90);
                newBorder2.name = "Block (" + (int)posToInstantiateNewBorder2.x + ", " + 1 + ", " +
                                  (int)posToInstantiateNewBorder2.z + ")";
                break;
            case 90:
                posToInstantiateNewBorder = new Vector3(posOfnewGo.x + 1, posOfnewGo.y, posOfnewGo.z);
                blockGrid[(int)posOfnewGo.x + 1, 1, (int)posOfnewGo.z] = (int)Enums.blockType.InsideRight1 + (int)(posToInstantiateNewBorder.z % (tileSize.y + tailleBridge));
                newBorder = Object.Instantiate(prefabs[blockGrid[(int)posOfnewGo.x + 1, 1, (int)posOfnewGo.z]],
                    new Vector3(posToInstantiateNewBorder.x - .5f,.5f, posToInstantiateNewBorder.z + .5f), Quaternion.identity);
                newBorder.transform.parent = parent.transform;
                newBorder.transform.Rotate(-90,0,0);
                newBorder.name = "Block (" + (int)posToInstantiateNewBorder.x + ", " + 1 + ", " + (int)posToInstantiateNewBorder.z + ")";
                
                posToInstantiateNewBorder2 = new Vector3(posOfnewGo.x + 4, posOfnewGo.y, posOfnewGo.z);
                blockGrid[(int)posOfnewGo.x + 4, 1, (int)posOfnewGo.z] = (int)Enums.blockType.InsideLeft1 + (int)(posToInstantiateNewBorder.z % (tileSize.y + tailleBridge));
                newBorder2 = Object.Instantiate(prefabs[blockGrid[(int)posOfnewGo.x + 4, 1, (int)posOfnewGo.z]],
                    new Vector3(posToInstantiateNewBorder2.x + .5f,.5f, posToInstantiateNewBorder2.z - .5f), Quaternion.identity);
                newBorder2.transform.parent = parent.transform;
                newBorder2.transform.Rotate(-90,180,0);
                newBorder2.name = "Block (" + (int)posToInstantiateNewBorder2.x + ", " + 1 + ", " + (int)posToInstantiateNewBorder2.z + ")";
                break;
            case 180:
                posToInstantiateNewBorder = new Vector3(posOfnewGo.x, posOfnewGo.y, posOfnewGo.z - 1);
                blockGrid[(int)posOfnewGo.x, 1, (int)posOfnewGo.z - 1] = (int)Enums.blockType.InsideBottom1 + (int)(posToInstantiateNewBorder.x % (tileSize.x + tailleBridge));
                newBorder = Object.Instantiate(prefabs[blockGrid[(int)posOfnewGo.x, 1, (int)posOfnewGo.z - 1]],
                    new Vector3(posToInstantiateNewBorder.x + .5f,.5f, posToInstantiateNewBorder.z + .5f), Quaternion.identity);
                newBorder.transform.parent = parent.transform;
                newBorder.transform.Rotate(-90,0,90);
                newBorder.name = "Block (" + (int)posToInstantiateNewBorder.x + ", " + 1 + ", " +
                                 (int)posToInstantiateNewBorder.z + ")";
                
                posToInstantiateNewBorder2 = new Vector3(posOfnewGo.x, posOfnewGo.y, posOfnewGo.z - 4);
                blockGrid[(int)posOfnewGo.x, 1, (int)posOfnewGo.z - 4] = (int)Enums.blockType.InsideTop1 + (int)(posToInstantiateNewBorder.x % (tileSize.x + tailleBridge));
                newBorder2 = Object.Instantiate(prefabs[blockGrid[(int)posOfnewGo.x, 1, (int)posOfnewGo.z - 4]],
                    new Vector3(posToInstantiateNewBorder2.x - .5f,.5f, posToInstantiateNewBorder2.z - .5f), Quaternion.identity);
                newBorder2.transform.parent = parent.transform;
                newBorder2.transform.Rotate(-90,-90,0);
                newBorder2.name = "Block (" + (int)posToInstantiateNewBorder2.x + ", " + 1 + ", " +
                                  (int)posToInstantiateNewBorder2.z + ")";
                break;
            case 270:
                posToInstantiateNewBorder = new Vector3(posOfnewGo.x - 1, posOfnewGo.y, posOfnewGo.z);
                blockGrid[(int)posOfnewGo.x - 1, 1, (int)posOfnewGo.z] = (int)Enums.blockType.InsideLeft1 + (int)(posToInstantiateNewBorder.z % (tileSize.y + tailleBridge));
                newBorder = Object.Instantiate(prefabs[blockGrid[(int)posOfnewGo.x - 1, 1, (int)posOfnewGo.z]],
                    new Vector3(posToInstantiateNewBorder.x + .5f,.5f, posToInstantiateNewBorder.z - .5f), Quaternion.identity);
                newBorder.transform.parent = parent.transform;
                newBorder.transform.Rotate(-90,180,0);
                newBorder.name = "Block (" + (int)posToInstantiateNewBorder.x + ", " + 1 + ", " + (int)posToInstantiateNewBorder.z + ")";
                
                posToInstantiateNewBorder2 = new Vector3(posOfnewGo.x - 4, posOfnewGo.y, posOfnewGo.z);
                blockGrid[(int)posOfnewGo.x - 4, 1, (int)posOfnewGo.z] = (int)Enums.blockType.InsideRight1 + (int)(posToInstantiateNewBorder.z % (tileSize.y + tailleBridge));
                newBorder2 = Object.Instantiate(prefabs[blockGrid[(int)posOfnewGo.x - 4, 1, (int)posOfnewGo.z]],
                    new Vector3(posToInstantiateNewBorder2.x - .5f,.5f, posToInstantiateNewBorder2.z + .5f), Quaternion.identity);
                newBorder2.transform.parent = parent.transform;
                newBorder2.transform.Rotate(-90,0,0);
                newBorder2.name = "Block (" + (int)posToInstantiateNewBorder2.x + ", " + 1 + ", " + (int)posToInstantiateNewBorder2.z + ")";
                break;
            default:
                throw new ArgumentOutOfRangeException();
        }
        blockGrid[(int)newGo.transform.position.x, (int)newGo.transform.position.y, (int)newGo.transform.position.z] = 0;
        blockHorizontalRotationGrid[(int)newGo.transform.position.x, (int)newGo.transform.position.y, (int)newGo.transform.position.z] = Enums.Side.none;
        blockVerticalRotationGrid[(int)newGo.transform.position.x, (int)newGo.transform.position.y, (int)newGo.transform.position.z] = Enums.Side.none;
        directionGrid[(int)newGo.transform.position.x, (int)newGo.transform.position.y, (int)newGo.transform.position.z] = Enums.Side.none;
    ...
    private bool SpecificBlockActions(Vector3 position, GameObject newGo)
    {
        ...
        switch (selectedPrefabIndex)
        {
            ...
            case 13:
                ...
                //offset for the position of the MakePlat  form function
    ...
    private GameObject PlacePanelEnd(GameObject newGo, GameObject newground, Vector3 posOfnewPanelStart, int rotation)
    {
        ...
        newPanelStart.name = "Block (" + posOfnewPanelStart.x + ", " + posOfnewPanelStart.y + ", " +
                             posOfnewPanelStart.z + ")";
    ...
    private bool RaycastNewBlockPos(Ray ray, GameObject blockHit, ref Vector3 position, out GameObject newGo)
    {
        ...
        if (selectedPrefabIndex != 13)
        {
            Debug.Log("name after:" + newGo.transform.name);
            newGo.name = newGo.transform.name = "Block (" + position.x + ", " + position.y + ", " + position.z + ")";
        }
        return false;
    }
    ...
    private void Delete()
    {
        ...
        switch (hitRay.transform.gameObject.transform.name)
        {
            ...
            case "PanelEnd(Clone)":
            {
                DeletePanel(hitRay.transform.gameObject);
                return;
            }
        }
                ...
                (int)hitRayPosition.z]))
        {
                        ...
                        (int)hitRayPosition.z])
            {
                ...
                case Enums.Side.back:
                    ...
                    break;
                case Enums.Side.left:
                    blockGrid[(int)hitRayPosition.x, (int)hitRayPosition.y, (int)hitRayPosition.z + 1] = 0;
                    blockHorizontalRotationGrid[(int)hitRayPosition.x, (int)hitRayPosition.y,
                        (int)hitRayPosition.z + 1] = Enums.Side.none;
                    blockVerticalRotationGrid[(int)hitRayPosition.x, (int)hitRayPosition.y,
                        (int)hitRayPosition.z + 1] = Enums.Side.none;
        ...
        for (int i = 0; i < blockGrid.GetLength(0); i++)
        {
            for (int j = 0; j < blockGrid.GetLength(1); j++)
            {
                for (int k = 0; k < blockGrid.GetLength(2); k++)
                {
                    switch (blockGrid[i, j, k])
                    {
                        case (int)Enums.blockType.M3_Block1:
    ...
    private void ReplaceBlock(int i, int j, int k, bool randomiseRotation)
    {
        ...
        {
            blockPrefab.transform.rotation = blockRotation;
        }
